...
from ..trainer.losses import rr_loss_functions, CrossEntropyLoss
...
class RRModel(nn.Module):
    def __init__(
        ...
        if train_args is not None:
            ...
            self.loss_fn = rr_loss_functions[self.loss_fn_str](self.margin)
    ...
    def forward(
        ...
        if neg_pairs is None:
            return pos_pair_scores
        neg_pair_scores = self.encode(neg_pairs)

        if pos_pair_scores.shape != neg_pair_scores.shape:
            return RROutput(
